...
class TestSerialization:
    def testPumpModelVersion1RoundTrip(self):
        ...
        serializedModelVersion1FilePath = (
            dataDirPath / "pumpModelVersion1.json"
        )
        expectedSerializedModelFilePath = (
            dataDirPath / "expectedPumpModel.json"
        )

        with serializedModelVersion1FilePath.open(
            mode="tr", encoding="UTF8"
        ) as file:
        ...
        expectedSerializedModel = expectedSerializedModelFilePath.read_text(
            encoding="UTF8"
        )
