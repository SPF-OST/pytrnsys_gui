...
class TestModel:
    ...
    def testConvertFromOldestToNewestFormat(self):
        ...
        storageTankLegacyJsonPath = (
            self.DATA_DIR_PATH / "storageTankOldestFormat.json"
        )
        storageTank = _stm.StorageTank.from_json(
            storageTankLegacyJsonPath.read_text()
        )
    ...
    def testRoundTripWithNewestFormat(self):
        newestFormatPath = (
            self.EXPECTED_DIR_PATH / "storageTankNewestFormat.json"
        )
